    ...
    <div className="px-3 lg:px-6 pt-10 pb-6 bg-bgColor dark:bg-darkBg">
          ...
          >
            <div className="flex justify-between">
              <div className="bg-white dark:bg-[#1E192A] h-[60px] p-3 w-[60px] icon-shadow rounded flex items-center justify-center">
                <span className="text-myBlue dark:text-myGreen ">
                  {task.icon}
                </span>
              </div>
              <div className="dropdown dropdown-end">
                <span
                  tabIndex={0}
                  className="w-[18px] h-[18px] text-myBlack dark:text-white cursor-pointer"
                >
                  {task.more}
                </span>
                <ul
                  tabIndex={0}
                  className="dropdown-content z-[1] menu p-2 shadow bg-white dark:bg-darkCard rounded-box "
                >
                  <li>
                    <Link
                      className="text-myBlack dark:text-myWhite rounded hover:text-white hover:bg-myBlue dark:hover:bg-myGreen"
                      href="#"
                    >
                      Delete
                    </Link>
            ...
            <h4 className="text-xl lg:text-2xl font-semibold text-myBlack dark:text-white mt-3 lg:mt-6 mb-3">
              {task.name}
            </h4>
            <Progress
              bgcolor={
                resolvedTheme === "dark" ? `${task.color1}` : `${task.color}`
              }
              completed={task.completed}
            />

            <p className="text-myGrey dark:text-[#D1D1D1] text-lg mt-3">
              {task.complete}
            </p>
          </div>
        ))}
      ...
      <div className="my-6 lg:flex gap-6">
              ...
              <div key={progress.id}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="w-full">
                    <h4 className="text-2xl font-semibold text-myBlack dark:text-white mb-3">
                      {progress.name}
                    </h4>
                    <Progress
                      bgcolor={
                    ...
                    <div className="flex justify-between mt-3">
                      <span className="text-myGrey dark:text-[#D1D1D1] text-lg">
                        {progress.complete}
                      </span>
                      <span className="text-myGrey dark:text-[#D1D1D1] text-lg">
                        {progress.parcent}
        ...
        <div className="lg:mt-0 mt-6 p-2 lg:py-8 lg:px-7 rounded bg-white dark:bg-darkCard h-full w-full ">
          <div className="flex flex-wrap gap-3 justify-between items-center">
            <h4 className="text-xl text-myBlack dark:text-myWhite">
              Tasks Reports
            </h4>
            <div>
              <Listbox value={selectedPerson} onChange={setSelectedPerson}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            {cards.map((card) => (
              ...
              >
                <div className="flex justify-between">
                  <h3 className="text-base text-myBlack dark:text-myWhite">
                    {card.name}
                  </h3>
                  <div className="group-hover:shadow-xl duration-500 w-10 h-10 bg-[#EDECFB] dark:bg-[#164C3C] flex items-center justify-center rounded-md">
                    <span className="mt-1 p-2 text-myBlue dark:text-myGreen">
                      {card.icon}
                    </span>
                  </div>
                </div>
                <div className="flex flex-wrap gap-4 mt-4">
                  <h3 className="text-[32px] text-myBlack dark:text-white font-semibold">
                    {card.view}
                  </h3>
                  <div
                  ...
                  >
                    ...
                    <span className="ml-1 hover:skew-y-12">
                      {card.trending}
                    </span>
                  </div>
                </div>
      ...
      <div>
        <div className="flex justify-between items-center">
          ...
          <p className="text-sm border dark:border-darkLine border-myLine text-myBlack dark:text-myWhite  py-3 px-4 rounded">
            Browse all tasks
          </p>
        </div>
        <div className="mt-6 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-6 w-full md:w-[454px] lg:w-full mx-auto">
          {taskReportCards.map((card) => (
            <div
              key={card.id}
              className="bg-white dark:bg-darkCard p-2 lg:py-8 lg:px-10 rounded  hover:-skew-y-3 duration-300"
            >
              <Image
                className="w-[120px] h-[120px] mx-auto mb-8"
                src={card.img}
                alt="buyer"
              />
              <h4 className="text-myBlack dark:text-white text-2xl font-semibold text-center mb-7">
                {card.title}
              </h4>
              <div className="flex flex-wrap gap-2 justify-around mb-8">
                <p className="text-myBlue dark:text-myGreen rounded py-1 px-5 md:px-8  lg:px-12 bg-[#F6F6FB] dark:bg-[#2C3241]">
                  {card.category}
                </p>
              ...
              <p className="text-lg text-center text-myBlack dark:text-[#D1D1D1]">
                {card.des}
              </p>
            </div>
          ))}
