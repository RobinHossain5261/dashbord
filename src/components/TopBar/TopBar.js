...
export default function TopBar({ showNav, setShowNav }) {
    ...
    <li className="flex items-center">
      ...
      <div className="dropdown ml-4 md:ml-6 lg:ml-10">
        <label tabIndex={0} className="">
          <span className="material-symbols-outlined cursor-pointer w-5 h-5 text-myBlack dark:text-myWhite">
            grid_view
          </span>
        </label>
        <div
          tabIndex={0}
          className="dropdown-content z-[1] menu p-2 mt-5 shadow bg-white dark:bg-[#1E192A] rounded-box w-52 md:w-52 lg:w-60"
        >
          <div className="flex flex-wrap justify-between items-center">
            <h3 className="text-xl font-semibold text-myBlack dark:text-white">
              Apps
            </h3>
            <span className="text-lg text-myGrey dark:text-myLine hover:text-myBlue dark:hover:text-myGreen cursor-pointer">
              View All Apps
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 ">
            {apps.map((app) => (
              ...
              >
                ...
                <span className="mt-2 text-xs dark:text-myWhite">
                  {app.name}
                </span>
              </div>
            ))}
      ...
      <div className="dropdown ml-4 md:ml-6 lg:ml-7">
        <label tabIndex={0} className="">
          <span className="material-symbols-outlined cursor-pointer text-myBlack dark:text-myWhite w-5 h-5">
            notifications
          </span>
        </label>
        <div
          tabIndex={0}
          className="dropdown-content z-[1] menu p-2 mt-5 shadow bg-white dark:bg-[#1E192A] rounded-box w-48 md:w-52 lg:w-60"
        >
          <div className="flex flex-wrap justify-between items-center">
            <h3 className="text-base sm:text-lg font-semibold text-myBlack dark:text-white">
              Notifications
            </h3>
            <span className="text-sm sm:text-base text-myGrey hover:text-myBlue dark:hover:text-myGreen cursor-pointer dark:text-myWhite">
              View All
            </span>
          </div>
          <hr className="my-3 dark:border-darkLine border-myLine" />
          <div className="">
            {notifications.map((notification) => (
              ...
              >
                ...
                />
                <div className="ml-2">
                  <h4 className="font-semibold text-sm sm:text-base flex-wrap text-myBlack dark:text-white">
                    {notification.title}
                  </h4>
                  <p className="dark:text-myWhite text-myBlack text-sm">
                    {notification.des}
                  </p>
                  <span className="dark:text-myWhite text-myBlack text-sm">
                    {notification.time}
                  </span>
                </div>
              </div>
      ...
      <div className="dropdown dropdown-end ml-4 md:ml-6 lg:ml-7">
        <label tabIndex={0} className="relative">
          ...
          <span className="absolute text-sm h-[18px] w-[18px]  rounded-full bg-myBlue dark:bg-myGreen flex justify-center items-center text-white -right-2 bottom-0">
            {carts.length}
          </span>
        </label>
        <div
          tabIndex={0}
          className="dropdown-content z-[1] menu p-2 mt-5 shadow bg-white dark:bg-[#1E192A] rounded-box w-52 md:w-52 lg:w-64"
        >
          <div className="flex justify-between">
            <h3 className="text-lg font-semibold text-myBlack dark:text-myWhite">
              My Cart
            </h3>
            <span className="text-myGrey dark:text-myWhite  hover:text-myBlue dark:hover:text-myGreen cursor-pointer ">
              {carts.length} Items
            </span>
          </div>
          <hr className="my-3 dark:border-darkLine border-myLine" />
          <div className="">
            {carts.map((cart) => (
              ...
              >
                ...
                <div className="ml-2">
                  <h4 className=" font-semibold text-base text-myBlack dark:text-white">
                    {cart.name}
                  </h4>
      ...
      <div className="dropdown dropdown-end ml-7 lg:ml-10">
        <div tabIndex={0} className="flex items-center cursor-pointer">
          ...
          <div className="lg:flex items-center hidden">
            <div>
              <p className="text-base text-myBlack dark:text-myWhite">
                Wade Warren
              </p>
              <span className="text-xs text-myBlack dark:text-myWhite">
                Account Setting
              </span>
            </div>
            <span className="material-symbols-outlined text-center w-5 h-5 dark:text-myWhite">
              expand_more
            </span>
          </div>
        </div>
        ...
        <ul
          tabIndex={0}
          className="dropdown-content z-[1] menu p-2 mt-5 shadow bg-white dark:bg-[#1E192A] rounded-box w-60"
        >
          <li className="mb-1 text-base ml-3 font-semibold text-myBlack dark:text-white">
            Welcome Wade!
          </li>
          <li>
            <Link
  ...
  return (
    <div
      className={`bg-white dark:bg-[#1E192A] left-0 py-6 z-[5] fixed w-full flex justify-between items-center transition-all duration-[400ms] ${
        showNav ? "lg:pl-[280px] md:pl-[280px] pl-[100%]" : ""
      }`}
    >
      <div className="flex items-center md:gap-5 lg:gap-[60px] py-2">
        <span
          className=" material-symbols-outlined ml-2  md:ml-6 lg:ml-[60px] cursor-pointer text-myBlack dark:text-myWhite"
        ...
        <form className="hidden md:hidden lg:flex items-center border dark:border-darkLine px-4 py-3 rounded">
          <input
            type="text"
            placeholder="Search..."
            className="outline-none  text-myBlack dark:text-myWhite dark:bg-[#1E192A]"
          />
          <span className="material-symbols-outlined cursor-pointer w-4 h-4 text-myBlack dark:text-myWhite">
            {" "}
            search{" "}
          </span>
        </form>
      ...
      <div className="mr-6">
        <ul className="flex relative">{menuItem}</ul>
      </div>
    </div>
  );
}
